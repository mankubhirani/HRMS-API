...
router.post("/timesheet_document", upload.single("profile"), function (req, res) {
  ...
  if (!req.file) {
    ...
    console.log(req);
    var sql =
      "INSERT INTO `timesheet_attachment` (`name`, `type`, `size`, `path`, `originalname`,`document_name`,`employeeId`,`company_id`) VALUES ('" +
      req.file.filename +
      "', '" +
      req.file.mimetype +
      "', '" +
...
router.get('/:employeeId/:companyId/:month/:year', attachmentController.findById);
